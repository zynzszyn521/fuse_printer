...
class _MyAppState extends State<MyApp> {
  ...
  final Map<String, Map<String, int>> _printerDevices = {
    '佳博 3150T': {'vendorId': 19267, 'productId': 13624},
    '其他打印机1': {'vendorId': 35329, 'productId': 59688},
    '其他打印机2': {'vendorId': 7925, 'productId': 512},
    '其他打印机3': {'vendorId': 10473, 'productId': 394},
  };

  String _selectedPrinter = '佳博 3150T';
  ...
  Future<void> printText() async {
    ...
    try {
      final text = _textController.text.isEmpty ? '测试打印文本' : _textController.text;
      final success = await FusePrinter.printText(text: text, align: 'center', size: 1);
      if (success != null && success) {
  ...
  Future<void> printBarcode() async {
    ...
    try {
      final code = _barcodeController.text.isEmpty ? '1234567890' : _barcodeController.text;
      final success = await FusePrinter.printBarcode(code: code, type: 'CODE128', height: 100);
      if (success != null && success) {
        await FusePrinter.printFeedPaper(lines: 3);
        showSnackbar('条码打印成功');
      } else {
  ...
  Future<void> printQRCode() async {
    ...
    try {
      final content = _qrCodeController.text.isEmpty ? 'https://example.com' : _qrCodeController.text;
  ...
  Future<void> printTestPage() async {
    ...
    try {
      // 打印标题
      await FusePrinter.printText(text: '=== 打印测试页面 ===', align: 'center', size: 2);
      await FusePrinter.printFeedPaper();
      
      // 打印文本
      await FusePrinter.printText(text: '这是一段测试文本', align: 'left', size: 1);
      await FusePrinter.printFeedPaper();
      
      // 打印条码
      await FusePrinter.printBarcode(code: 'TEST123456', type: 'CODE128', height: 80);
      await FusePrinter.printFeedPaper();
      
      // 打印二维码
      await FusePrinter.printQRCode(content: 'https://example.com/test', size: 8);
      await FusePrinter.printFeedPaper(lines: 3);
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          title: const Text('Fuse Printer 测试应用'),
        ),
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('打印机状态: $_printerStatus', style: TextStyle(color: _isPrinterConnected ? Colors.green : Colors.red)),
              ...
              DropdownButtonFormField<String>(
                ...
                items: _printerDevices.keys.map((String device) {
